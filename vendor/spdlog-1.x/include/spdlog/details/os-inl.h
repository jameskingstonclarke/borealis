...
SPDLOG_INLINE void utf8_to_wstrbuf(string_view_t string, wmemory_buf_t &target)
{
    if (string.size() > static_cast<size_t>(std::numeric_limits<int>::max()) - 1)
    {
        throw_spdlog_ex("UTF-8 string is too big to be converted to UTF-16");
    }

    int str_size = static_cast<int>(string.size());
    if (str_size == 0)
    {
        target.resize(0);
        return;
    ...
    {
        result_size = ::MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, string.data(), str_size, NULL, 0);
    }

    if (result_size > 0)
    {
        target.resize(result_size);
        result_size = ::MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, string.data(), str_size, target.data(), result_size);
